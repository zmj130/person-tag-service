...
import { Plus, Search, RotateCcw, Tags, UserRound, X } from 'lucide-vue-next'
import { api } from '../api'
...
const tags = ref([])
const selected = ref([])
const filters = reactive({ keyword: '', tagIds: [], tagOperator: 'AND', pageNo: 1, pageSize: 20 })
...
function reset() { filters.keyword = ''; filters.tagIds = []; filters.tagOperator = 'AND'; search() }
...
async function openDetail(row) {
  ...
  try { currentBindings.value = await api.personTags(row.id) }
...
async function unbind(binding) {
  await ElMessageBox.confirm(`确认解绑“${tagMap.value[binding.tagId]?.name || '该标签'}”？`, '解绑标签', { type: 'warning' })
...
<template>
  <section class="workspace">
    ...
    <div class="table-wrap">
      <el-table v-loading="loading" :data="rows" row-key="id" height="100%" empty-text="暂无人员数据" @selection-change="selected = $event">
        ...
        <el-table-column label="操作" width="130" fixed="right"><template #default="{ row }"><el-button link type="primary" @click="openDetail(row)">查看</el-button><el-button link @click="openEdit(row)">编辑</el-button></template></el-table-column>
      </el-table>
    </div>
  ...
  <el-drawer v-model="detailOpen" size="min(560px, 96vw)" :title="currentPerson?.name || '人员详情'">
    <template v-if="currentPerson">
      ...
      <div class="section-heading"><h3>人员标签</h3><span>{{ currentBindings.length }} 个</span></div>
      ...
      <div v-loading="detailLoading" class="binding-list">
        <div v-for="binding in currentBindings" :key="binding.id" class="binding-row"><div><b>{{ tagMap[binding.tagId]?.name || binding.tagId }}</b><p><StateTag :value="binding.source" /> <StateTag :value="binding.status" /><span v-if="binding.matchedKeyword">命中“{{ binding.matchedKeyword }}”</span></p></div><el-button v-if="binding.source === 'MANUAL'" text circle title="解绑标签" aria-label="解绑标签" @click="unbind(binding)"><X :size="17" /></el-button></div>
        <el-empty v-if="!detailLoading && !currentBindings.length" description="暂未绑定标签" :image-size="70" />
      </div>
    </template>
  </el-drawer>
</template>
